...
const verifyPayment = async (req, res, next) => {
  try {
    ...
    if (result.alreadyProcessed) return ApiResponse.success(res, result, 'Payment already processed');
    return ApiResponse.success(res, result, 'Payment verified and subscription activated');
  } catch (error) {
    if (error.message.includes('Invalid signature')) return ApiResponse.error(res, 'Payment verification failed', 400);
...
const handleWebhook = async (req, res, next) => {
  try {
    ...
    if (!isValid) return ApiResponse.error(res, 'Invalid webhook signature', 400);
